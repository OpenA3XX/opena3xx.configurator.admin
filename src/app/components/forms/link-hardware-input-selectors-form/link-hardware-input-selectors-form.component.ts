...
export class LinkHardwareInputSelectorsFormComponent{
  ...
  fetchData(){
    this.dataService.getAllIntegrationTypes().toPromise()
    .then((integrationTypes: OptionList[])=>{
      this.integrationTypeFieldConfig.options = integrationTypes;
    })
    .then(() =>{
      return this.dataService.getHardwareInputSelectorDetails(this.hardwareInputSelectorId).toPromise()
    })
    .then((hardwareInputSelectorDetails: any) =>{
      if(hardwareInputSelectorDetails.simulatorEventDto != undefined){
        this.linkHardwareInputSelectorsForm.controls["integrationTypes"].setValue(hardwareInputSelectorDetails.simulatorEventDto.simulatorEventSdkType);
        return hardwareInputSelectorDetails;
      }
    })
    .then((hardwareInputSelectorDetails: any)=>{
      if(hardwareInputSelectorDetails != undefined){
        this.dataService.getAllSimulatorEventsByIntegrationType(hardwareInputSelectorDetails.simulatorEventDto.simulatorEventSdkType).toPromise().then((data: SimulatorEventItemDto[])=>{
          _.each(data,(entry)=>{
  ...
  onSubmit(formData: any) {
    if(this.linkHardwareInputSelectorsForm.valid){
      ...
      console.log("hardwareInputDto", this.hardwareInputDto, "HardwareInputSelectorId", this.hardwareInputSelectorId, "FormData", this.linkHardwareInputSelectorsForm.value)
    }
    else{
  ...
  onIntegrationTypeChange(selectChangeEvent: any){
    ...
    this.dataService.getAllSimulatorEventsByIntegrationType(selectChangeEvent.value).toPromise().then((data: SimulatorEventItemDto[])=>{
        _.each(data,(entry)=>{
          this.simulatorEventsFieldConfig.options.push({
            key: entry.id.toString(),
            value: `${entry.eventName} => ${entry.eventCode}`
